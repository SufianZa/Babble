...
public final class DB_query implements Closeable {
    ...
    public ArrayList<Babble> getOwnBabble(String username) {
        ArrayList<Babble> result = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement("select b.id, b.text, b.created, b.creator from dbp66.babble b where b.creator =?")) {
            ps.setString(1, username);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    int id = rs.getInt(1);
    ...
    public ArrayList<Message> getMessages(String writer, String receiver) {
        ...
        String sql = "select m.text, m.created, m.id , m.sender, m.recipient from DBP66.Babblemessage m where (m.sender = ? and m.recipient = ?) or (m.sender = ? and m.recipient = ?)";
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                while (rs.next()) {
                    result.add(new Message(rs.getInt(3),
                            rs.getString(1),
                            rs.getString(4),
                            rs.getString(5),
                            rs.getTimestamp(2)));
